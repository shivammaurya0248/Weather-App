...
function calculateProgress(sunriseProp: string, sunsetProp: string): number {
    ...
    if (total_duration <= 0) {
        return now.isAfter(sunrise) ? 1 : 0;
    }
...
function SimpleCurve(props: SimpleCurveProps) {
    ...
    const [progress, setProgress] = React.useState(() => calculateProgress(props.sunrise, props.sunset));

    React.useEffect(() => {
        const updateProgress = () => {
            setProgress(calculateProgress(props.sunrise, props.sunset));
        };
        
        updateProgress(); // Update immediately on mount and when props change.
